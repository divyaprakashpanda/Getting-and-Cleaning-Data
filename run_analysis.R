...
# Creating a single data frmae for Training set from 3 different files.
training_data = cbind(y_train,subject_train,x_train);

# Creating a single data frmae for test set from 3 different files.
...
# Merge the final_data set with the acitivity_type dataframe
...
# Cleaning up the column names
  names(final_data)<-gsub("^t", "time", col_names)
  names(final_data)<-gsub("^f", "frequency", col_names)
  names(final_data)<-gsub("Acc", "Accelerometer", col_names)
  names(final_data)<-gsub("Gyro", "Gyroscope", col_names)
  names(final_data)<-gsub("Mag", "Magnitude", col_names)
  names(final_data)<-gsub("BodyBody", "Body", col_names)
